...
class BuyStructureButton(Button):
    def __init__(self, game, rect: pg.Rect, building_id: str):
        ...
        self.name_text = Text(game, "fonts/pixel-bit-advanced.ttf", 12, (0, 0, 0),
                              pg.Vector2(rect.center[0], rect.y + 12), justification=JUSTIFY_CENTER)
    ...
    def draw(self):
        super().draw()
        self.game.screen.blit(self.game.tile_library[self.building_id].sprite,
                              (self.rect.x + TILE_WIDTH / 2, self.rect.y + TILE_HEIGHT / 2))
        self.name_text.draw()
class Panel:
