PLAY_STATE = 0
EDIT_STATE = 1
BUY_STATE = 2
INVENTORY_STATE = 3
FRIDGE_STATE=4
MAIN_MENU_STATE = 5
BUY_ITEM_STATE = 6 # THIS IS A TRAVESTY
DELETE_STATE = 7
MOVE_STATE = 8

COOK_WAVE = 0
BREAK_WAVE = 1
TRANSITION_WAVE = 2

PPU = 2 # PIXELS PER UNIT
UPT = 16 # UNIT PER TILE

TILE_WIDTH = UPT * PPU
TILE_HEIGHT = UPT * PPU

WORLD_WIDTH = 16
WORLD_HEIGHT = 12
WORLD_WALL_HEIGHT = 3

STATS_MARGIN = TILE_WIDTH * 4

MARGIN = TILE_WIDTH

SCREEN_WIDTH = WORLD_WIDTH * TILE_WIDTH + 2*MARGIN
SCREEN_HEIGHT = STATS_MARGIN + (WORLD_HEIGHT + WORLD_WALL_HEIGHT) * TILE_HEIGHT + 2*MARGIN


MASTER_VOLUME = 100

BACKGROUND_COLOUR = (33, 33, 35)

PLAYER_HITBOX_HEIGHT = 7/16
PLAYER_HITBOX_WIDTH = 7/16

PLAYER_MAX_SPEED = 4.4
PLAYER_TIME_TO_MAX = 0.02
PLAYER_ACCELERATION  = PLAYER_MAX_SPEED / PLAYER_TIME_TO_MAX
PLAYER_DECELERATION_TIME = 0.08
PLAYER_DECELERATION = PLAYER_MAX_SPEED / PLAYER_DECELERATION_TIME

DASH_POWER = 25
TIME_TO_TAKE_DASH = 0.08

STARTING_MONEY = 500

BUY_BUILDINGS = ["counter", "fridge", "chopper", "oven", "seller", "item-shop"]
BUY_ITEMS = ["sugar", "flour", "butter", "chocolate"]